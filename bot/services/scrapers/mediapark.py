...
class MediaparkScraper(BaseScraper):
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=headers, timeout=timeout) as s:
                async with s.get(proxy) as resp:
                    if resp.status != 200:
                        logger.warning(f"Mediapark: HTTP {resp.status}")
                        return []
                    html = await resp.text(errors="replace")
        except Exception as e:
            logger.error(f"Mediapark fetch: {e}")
            return []
    ...
    def _parse_cards(self, soup: BeautifulSoup) -> List[ProductResult]:
        results = []
        selectors = [
            "div.product-card", "div.product-item", "div.goods-item",
            "li.product-card", "li.product-item",
            "[class*='product-card']", "[class*='product-item']",
            "[class*='goods-item']", "[class*='catalog-item']", "article",
        ]
        cards = []
